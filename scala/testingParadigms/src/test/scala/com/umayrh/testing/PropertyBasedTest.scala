 ...
 */
class PropertyBasedTest extends FeatureSpec with AssertionsForJUnit with GivenWhenThen with GeneratorDrivenPropertyChecks with Matchers {
    ...
    feature("A summing function for sequences of integers - tested using function properties") {
        scenario("the function is commutative") {
            ...
            val inputSeqGen = Gen.listOf(choose(Int.MinValue, Int.MaxValue)).suchThat(_.size >= 0)

            When("reducer is invoked on the sequence and its shuffled version")
            Then("result is the equal if there no overflow/underflow, ArithmeticException thrown otherwise")
            ...
            forAll(inputSeqGen) {
                inputSeq: List[Int] =>
                    ...
                    try {
                        resultShuffled = ReducerRedux.reduceSeq(scala.util.Random.shuffle(inputSeq), ReducerRedux.sumLongs)
                    } catch {
        ...
        scenario("the function is associatative") {
            ...
            forAll(inputSeqGen) {
                inputSeq: List[Int] =>
                    ...
                    try {
                        val inputSeqSplit = inputSeq.splitAt(splitIndex).productIterator.toList
                        val inputSeqSplitResult = inputSeqSplit.map(_.asInstanceOf[List[Int]]).map(seq => ReducerRedux.reduceSeq(seq, ReducerRedux.sumLongs))
