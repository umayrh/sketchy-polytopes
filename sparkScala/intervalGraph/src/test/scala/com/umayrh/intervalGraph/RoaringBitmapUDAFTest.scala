...
object RoaringBitmapUDAFTest {
  ...
  val UDAF = new RoaringBitmapUDAF(INPUT_COL)
}
...
class RoaringBitmapUDAFTest
  ...
  Feature(
    ...
    Scenario("The UDAF is commutative ") {
      ...
      val expectedDf = df.agg(UDAF(df(INPUT_COL)))

      When("bitmaps rows in a dataframe are shuffled")
      val shuffledDf = df.orderBy(rand())
      val actualDf = shuffledDf.agg(UDAF(shuffledDf(INPUT_COL)))
    ...
    Scenario("The UDAF is associative ") {
      ...
      val data = inputData()
      val df = toDf(data)
      val expectedDf = df.agg(UDAF(df(INPUT_COL)).as(OUTPUT_COL))
      ...
      val bitmapsDf =
        splitDf.map(df => df.agg(UDAF(df(INPUT_COL)).as(OUTPUT_COL)))
      val unionedDf = Random.shuffle(bitmapsDf).reduce(_.union(_))
      val actualDf = unionedDf.agg(UDAF(unionedDf(OUTPUT_COL)))
    ...
    Scenario("The UDAF is idempotent") {
      ...
      Range(0, numIter).foreach(k =>
        actualDf = actualDf.agg(UDAF(actualDf(OUTPUT_COL)).as(OUTPUT_COL)))
    ...
    Scenario("The UDAF obeys De Morgan's law: A OR B = ~A AND ~B ") {
      ...
      val df = toDf(data)
      val actualDf = df.agg(UDAF(df(INPUT_COL)).as(INPUT_COL))
